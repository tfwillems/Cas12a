...
flow_cutoff <- quantile(flow_lfcs$avg_lfc, 0.01)
...
ggplot(eef2_tiers_percent_active) +
  aes(x = Tier, y = fraction_active, label = n, fill = Tier) +
  geom_bar(aes(alpha = Tier), stat = 'identity', color = 'black', show.legend = FALSE) +
  ...
  xlab('PAM') + 
  ylab('Fraction Active') +
  scale_fill_manual(values = c('grey','#4878d0', '#4878d0', '#ee854a', '#6acc64', 'darkgrey')) +
  scale_alpha_manual(values = c(0.5,0.5,1,1,1,1))
...
ggsave(here('figures','ML','EEF2_guide_activity.pdf'), width = 9, height = 9, units = 'cm', useDingbats = FALSE)
```
...
```{r}
test = eef2_binarized %>%
  filter(sequence_type == 'Orig', Tier == 'TTTN') 
sum(test$Avg_LFC < flow_cutoff)
nrow(test)
25/40

# vs
test = eef2_binarized %>%
  filter(sequence_type == 'Orig', Tier == 'Tier 1')
sum(test$Avg_LFC < flow_cutoff)
nrow(test)
150/222
```
...
ggplot(tier_grouped_correlations) +
  ...
  ylab('Correlation')
ggsave(here('figures','ML','EEF2_predictions.pdf'), width = 9, height = 6, units = 'cm', 
       useDingbats = FALSE)
```
...
ggsave(here('figures', 'ML','hacked_pam_predictions.pdf'), 
       width = 12, height = 9, units = 'cm', useDingbats=FALSE)
```
