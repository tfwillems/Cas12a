...
ggsave(here('figures','synlet', 'single_gene_effect.pdf'), width = 6, height = 12, units = 'cm')
```
...
plot_model_fit <- function(base_lfcs, curr_assay) {
  ...
  p <- ggplot(assay_lfcs) +
    ...
    scale_color_manual(values = c('#ee854a', '#956cb4')) +
...
write_csv(guide_residuals, here('processed_data', '2019-08-28_synlet_guide_residuals.csv'))
```
...
for (curr_pair in unique(unlist(compare_residuals[!(compare_residuals$control1 | compare_residuals$control2), 'pair']))) {
  for (curr_cell in unique(compare_residuals$assay)) {
    ...
    ggplot(current_residuals) +
      ...
      ggtitle(paste0(curr_pair, ' ', curr_cell)) +
    ...
    ggsave(here('figures','synlet','system_comparison', paste0(curr_pair, '_',
                                                               curr_cell, '.pdf')), 
           useDingbats = F, width = 4.5, height = 6, units = 'cm')
  }
}
```
