...
achilles_common_essentials <- read_csv(here('data', 'raw', 'Achilles_common_essentials.csv')) %>%
  mutate(gene_name = word(gene,1), gene_id =  str_extract(gene, '(?<=\\()[0-9]*'))
...
melted_lfcs <- melted_lognorms %>%
  ...
  separate(assay, by = '_', c('assay', 'rep'))
```
...
ggsave(here('figures', 'synlet', 'A375_replicate_correlations.pdf'),
...
melted_lfcs = rep_lfcs %>%
  ...
  mutate(control1 = grepl('OR', gene1), 
         ...
         DR_name = recode(DR, 'TAATTTCTACTGTCGTAGAT' = 'DR1', 
...
ggsave(here('figures','synlet','olfactory_outliers.pdf'),
       width = 10, height = 6, units = 'cm', useDingbats = FALSE)
```
...
ovc_p_lfcs$p
ggsave(here('figures', 'synlet', 'ovcar_position_correlations.pdf'),
       width = 8, height = 6, units = 'cm', useDingbats = FALSE)
a375_p_lfcs <- plot_position_cor(target_position_LFCs %>% filter(assay == 'A375'), 'A375')
a375_p_lfcs$p
ggsave(here('figures', 'synlet', 'A375_position_correlations.pdf'),
       width = 10, height = 6, units = 'cm', useDingbats = FALSE)
```
...
write_csv(all_guide_lfcs, here('tables', 'synlet', 'positional_effects.csv'))
```
...
plot_model_fit <- function(base_lfcs, curr_assay) {
  ...
  p <-  ggplot(assay_lfcs) +
    ...
    geom_point_rast(alpha = 0.4, aes(color = type), raster.width = 3.2, raster.height = 3.2, 
                    raster.dpi = 600) +
    theme_classic() +
    theme(text = element_text(size = 10, family = 'Arial'), 
...
ggsave(here('figures','synlet','a375_linear_model.pdf'), useDingbats = FALSE, 
       width = 8, height = 8, units = 'cm')
plot_model_fit(base_lfcs, 'OVCAR8')
ggsave(here('figures','synlet','ovcar8_linear_model.pdf'), useDingbats = FALSE, 
       width = 8, height = 8, units = 'cm')
...
ggsave(here('figures', 'synlet', 'A375_residual_dist.pdf'), useDingbats = FALSE, 
       width =9, height = 9, units = 'cm')
syn_active_all_ovcar8 <- plot_residual_dist(named_residuals, 'OVCAR8')
syn_active_all_ovcar8$p
ggsave(here('figures', 'synlet', 'ovcar8_residual_dist.pdf'), useDingbats = FALSE, 
       width =9, height = 9, units = 'cm')
```
...
target_guide_ranks <- base_lfcs %>%
  ...
  group_by(gene1) %>%
  mutate(rank = rank(-`Seq-DeepCpf1 Score`),
         quartile = cut(rank, breaks = c(0,5,10,15,20))) %>%
  rename(guide=guide1) %>%
  ungroup() %>%
  rename(gene = gene1)
...
cas12a_residual_designs <- named_residuals %>%
  ...
  left_join(target_guide_ranks, by = c('guide2' = 'guide', 'gene2' = 'gene'), 
             suffix = c('.1','.2'))
top_quartile_residuals <- cas12a_residual_designs %>%
  filter((`Seq-DeepCpf1 Score.1` > 50 & `Seq-DeepCpf1 Score.2` > 50) | control1 | control2)
syn_active_filt_a375 <- plot_residual_dist(top_quartile_residuals, 'A375')
syn_active_filt_a375$p
ggsave(here('figures', 'synlet', 'a375_filt_residual_dist.pdf'), 
       width =9, height = 9, units = 'cm')
syn_active_filt_ovcar8 <- plot_residual_dist(top_quartile_residuals, 'OVCAR8')
syn_active_filt_ovcar8$p
ggsave(here('figures', 'synlet', 'ovcar_filt_residual_dist.pdf'), 
       width =9, height = 9, units = 'cm')
```
...
plot_diff_syn <- function(all_syn, filtered_syn) {
  ...
  p = ggplot(increased_activity) +
    ...
    xlim(0,1) +
    ylim(0,1) +
    xlab('Fraction Synthetic\nAll Pairs') +
    ylab('Fraction Synthetic\nSeq-DeepCpf1 > 50') +
    theme_classic() +
    theme(text = element_text(size = 10, family = 'Arial'), 
          plot.title = element_text(size = 10, face = 'bold')) +
...
ggsave(here('figures', 'synlet', 'ovcar8_on_targ_filt.pdf'), useDingbats = F, 
       width = 8, height = 8, units = 'cm')
plot_diff_syn(syn_active_all_a375$percent_syn, syn_active_filt_a375$percent_syn) +
  ggtitle('A375 Fraction Synthetic')
ggsave(here('figures', 'synlet', 'a375_on_targ_filt.pdf'), useDingbats = F, 
       width = 8, height = 8, units = 'cm')
```
...
cas12a_compare_designs <- cas12a_designs %>% 
...
melted_compare_residuals <- compare_residuals %>%
  filter(!(control1 | control2)) %>%
  gather(type,`Avg. LFC`, `Avg. LFC`, `Expected LFC`) %>%
  mutate(type = fct_recode(type,'Expected' = 'Expected LFC', 'Observed' = 'Avg. LFC'), 
         type = factor(type, levels = c('Expected', 'Observed')))
```

```{r}
ggplot(melted_compare_residuals) +
  aes(x = pair,fill = type, y = `Avg. LFC`) +
  facet_grid(c('system', 'assay'), scales = 'free_y') +
  geom_boxplot() +
  theme_classic() +
  theme(text = element_text(size = 10, family = 'Arial'),
...
ggsave(here('figures','synlet','en_bp_cor.pdf'),en_bp_cor, width = 9, height = 6, units = 'cm', 
       useDingbats = FALSE)
```
