...
rep_spread_cas9 <- all_cas9 %>%
  gather(treatment, LFC, `A375|RDA_085|no drug|NA|pDNA|A|LFC`:`A375|RDA_085|Talazaparib|High|no drug|B|LFC`) %>%
  separate(treatment, sep = '\\|', into = c('Cell', 'RDA', 'Condition', 'dose','from', 'Rep', 'Measure')) %>%
  spread(Rep, LFC) %>%
  mutate(Avg.LFC = (A+B)/2)
...
```{r}
count(dropout_2x, Gene, RDA)
```

```{r}
cell_cell_cors <- duplicate_lfcs %>%
  group_by(Cell.x, Cell.y) %>%
  summarise(Pearson = cor(Avg.LFC.x, Avg.LFC.y)) %>%
  group_by(Cell.x) %>%
  mutate(Cell.x.n = n()) %>%
  group_by(Cell.y) %>%
  mutate(Cell.y.n = n()) %>%
  ungroup()%>%
  mutate(Cell.x = fct_reorder(Cell.x, -Cell.x.n), 
         Cell.y = fct_reorder(Cell.y, -Cell.y.n))
```

```{r}
ggplot(cell_cell_cors) +
  aes(x = Cell.x, y = Cell.y) +
  geom_point(aes(size = Pearson, color = Pearson)) +
  geom_text(aes(label = round(Pearson, 2)), size = 2.83, color = 'white') +
  ...
  xlab('') +
  ylab('') 
ggsave(here('figures','tiling','essential_cors.pdf'),useDingbats = FALSE, width = 6, height = 6, 
       units = 'cm')
```
...
cell_x = 'A549'
cell_y = 'A375'
example_plot = duplicate_lfcs %>%
  filter(Cell.x == cell_x, Cell.y == cell_y)
```

```{r}
ggplot(example_plot) +
  aes(x = Avg.LFC.x, y = Avg.LFC.y) +
